...
class StepBuilder:
  ...
  def __init__(self,
    ...
    Args:
      ...
      dynamic_inputs: A dictionary mapping upstream component output names to
        types used to resolve Placeholder authoring in downstream components.
  ...
  def build(self) -> Dict[str, pipeline_pb2.PipelineTaskSpec]:
    ...
    for name, output_channel in self._outputs.items():
      ...
      # Update 1: Output Parameter Type
      if self._dynamic_inputs:
        if name in self._dynamic_inputs[self._node.id]:
          component_def.output_definitions.parameters[
              name].parameter_type = self._dynamic_inputs[self._node.id][name]
