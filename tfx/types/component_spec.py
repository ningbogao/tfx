...
class ComponentSpec(json_utils.Jsonable):
  ...
  @classmethod
  def is_input_channel_input_optional(cls, key: str) -> bool:
    """Returns whether the input channel is input_optional."""
    try:
      return cast(ChannelParameter, cls.INPUTS[key]).input_optional
    except KeyError as e:
      raise KeyError(f'self.INPUTS = {cls.INPUTS}') from e
...
class ChannelParameter:
  ...
  def __init__(
      ...
      optional: bool = False,
      input_optional: Optional[bool] = None):
    """ChannelParameter constructor.

    Note the distinction between `optional` and `input_optional`.
    ...
    before the component is triggered (i.e. set `input_optional` to False).

    Args:
      type: Artifact type of artifacts in this channel.
      optional: Whether providing this channel is optional.
      input_optional: Optional. Whether having inputs to this channel is
        optional. Should only be explicitly set if `optional` is True.
    """
    ...
    if input_optional is not None:
      if not optional:
        raise ValueError('input_optional should only be explictly set if '
                         'optional is True')
    self.input_optional = input_optional
    if not optional:
      # Input is always mandatory if channel is mandatory
      self.input_optional = False
  ...
  def __eq__(self, other):
    return (isinstance(other.__class__, self.__class__) and
            other.type == self.type and other.optional == self.optional and
            other.input_optional == self.input_optional)
